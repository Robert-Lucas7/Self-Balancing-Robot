...
def generate_launch_description():
    ...
    urdf_file_name = 'urdf/model_V2.xacro'
    ...
    world_path = os.path.join(get_package_share_directory('sbr_pkg'), 'world', world_file_name)
    ...
    execute_gazebo_cmd = ExecuteProcess(
            cmd=['gazebo', '--verbose', '-s', 'libgazebo_ros_factory.so','-s', 'libgazebo_ros_init.so', world_path], #Change gzserver to gazebo for gui
            output='screen',
            )
    ...
    control_code_cmd = Node(
        package='sbr_pkg',
        executable='SelfBalance',#'control',
        output='screen'
        )
